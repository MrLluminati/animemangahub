...
import type { CatalogRelationGroup, CatalogTitle } from "@/types/catalog";
...
async function fetchRelationGroups(path: string): Promise<CatalogRelationGroup[]> {
  try {
    const response = await fetch(`${API_BASE_URL}${path}`, {
      next: { revalidate: 1800 }
    });
...
export function getAnimeRelations(id: string | number) {
  return fetchRelationGroups(`/api/anime/${id}/relations`);
}
...
export function getMangaRelations(id: string | number) {
  return fetchRelationGroups(`/api/manga/${id}/relations`);
}
